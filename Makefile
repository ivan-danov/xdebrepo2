...
define deb_changelog
	$(Q)git log --pretty --numstat --summary|git2cl > $(DEBDIR)/usr/share/doc/$(DEBNAME)/changelog
...
define deb_control
	...
	$(Q)echo "Depends: lsb-release, debconf (>= 0.5) | debconf-2.0, logrotate, cron $(PACKAGE_DEPS)" >> $(DEBDIR)/DEBIAN/control
...
define deb_func
	...
	${Q}cp xdebrepo2 $(DEBDIR)/usr/bin/xdebrepo2
	${Q}chmod 0755 $(DEBDIR)/usr/bin/xdebrepo2

	${Q}mkdir -p $(DEBDIR)/usr/share/bash-completion/completions
	${Q}cp xdebrepo2.bash_completion $(DEBDIR)/usr/share/bash-completion/completions/xdebrepo2
	${Q}chmod 0644 $(DEBDIR)/usr/share/bash-completion/completions/xdebrepo2
